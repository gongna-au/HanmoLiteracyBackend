...
func (c *CharacterModel) Create() error {
	return db.DB.
		Table("tbl_character").
		Create(c).Error
}

func (c *CharacterModel) Save() (err error) {
	return db.DB.
		Table("tbl_character").
		Save(c).Error
}

func CharacterInit() error {
	characters := `"
	一 乙 
	...
	characterArray := reg.FindAllString(characters, -1)
	...
	for _, v := range characterArray {
		...
		err := db.DB.
			...
			Save(&ch).Error
		if err != nil {
			return errors.New("insert character err")
		}
...
func GetCharacterIdByName(name string) (*CharacterModel, error) {
	c := &CharacterModel{}
	d := db.DB.
		Table("tbl_character").
		Where("name = ?", name).First(c)
	return c, d.Error
}
...
func RegexpCompile(str string) []string {
	reg, _ := regexp.Compile(`[\p{Han}]+`) // 查找连续的非ascii字符串
	characterArray := reg.FindAllString(str, -1)
	return characterArray
}

func JudgeDefaultCharacterIdExist() int64 {
	var count int64
	db.DB.
		Table("tbl_character").
		Where("id > ?", 0).Count(&count)
	return count
}
